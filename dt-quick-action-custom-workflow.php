 ...
 * 1. @todo Rename the `disciple-tools-quick-action-custom-workflow.php file.
 * 2. @todo Refactor all occurrences of the name Disciple_Tools_Quick_Action_Custom_Workflow, disciple_tools_quick_action_custom_workflow, disciple-tools-quick-action-custom-workflow, starter_post_type, and "Plugin Starter Template"
...
/**
 * Gets the instance of the `Disciple_Tools_Quick_Action_Custom_Workflow` class.
 *
 * @since  0.1
 * @access public
 * @return object|bool
 */
function disciple_tools_quick_action_custom_workflow() {
    $disciple_tools_quick_action_custom_workflow_required_dt_theme_version = '1.19';
    ...
    if ( $is_theme_dt && version_compare( $version, $disciple_tools_quick_action_custom_workflow_required_dt_theme_version, '<' ) ) {
        add_action( 'admin_notices', 'disciple_tools_quick_action_custom_workflow_hook_admin_notice' );
    ...
    return Disciple_Tools_Quick_Action_Custom_Workflow::instance();

}
add_action( 'after_setup_theme', 'disciple_tools_quick_action_custom_workflow', 20 );

//register the D.T Plugin
add_filter( 'dt_plugins', function ( $plugins ){
    $plugin_data = get_file_data( __FILE__, [ 'Version' => 'Version', 'Plugin Name' => 'Plugin Name' ], false );
    $plugins['disciple-tools-quick-action-custom-workflow'] = [
 ...
 * @access public
 */
class Disciple_Tools_Quick_Action_Custom_Workflow {
    ...
    private function __construct() {
         ...
         */
        if ( $is_rest && strpos( dt_get_url_path(), 'disciple-tools-quick-action-custom-workflow' ) !== false ) {
         ...
         */
        // if ( strpos( dt_get_url_path(), 'metrics' ) !== false || ( $is_rest && strpos( dt_get_url_path(), 'disciple-tools-quick-action-custom-workflow-metrics' ) !== false ) ){
...
//        require_once( 'magic-link/magic-link-home.php' );
    ...
    public static function deactivation() {
        ...
        delete_option( 'dismissed-disciple-tools-quick-action-custom-workflow' );
    }
    ...
    public function i18n() {
        $domain = 'disciple-tools-quick-action-custom-workflow';
    ...
    public function __toString() {
        return 'disciple-tools-quick-action-custom-workflow';
    }
    ...
    public function __call( $method = '', $args = array() ) {
        _doing_it_wrong( 'disciple_tools_quick_action_custom_workflow::' . esc_html( $method ), 'Method does not exist.', '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
register_activation_hook( __FILE__, [ 'Disciple_Tools_Quick_Action_Custom_Workflow', 'activation' ] );
register_deactivation_hook( __FILE__, [ 'Disciple_Tools_Quick_Action_Custom_Workflow', 'deactivation' ] );


if ( ! function_exists( 'disciple_tools_quick_action_custom_workflow_hook_admin_notice' ) ) {
    function disciple_tools_quick_action_custom_workflow_hook_admin_notice() {
        global $disciple_tools_quick_action_custom_workflow_required_dt_theme_version;
        $wp_theme = wp_get_theme();
        ...
        if ( $wp_theme->get_template() === 'disciple-tools-theme' ){
            $message .= ' ' . sprintf( esc_html( 'Current Disciple.Tools version: %1$s, required version: %2$s' ), esc_html( $current_version ), esc_html( $disciple_tools_quick_action_custom_workflow_required_dt_theme_version ) );
        }
        // Check if it's been dismissed...
        if ( ! get_option( 'dismissed-disciple-tools-quick-action-custom-workflow', false ) ) { ?>
            <div class="notice notice-error notice-disciple-tools-quick-action-custom-workflow is-dismissible" data-notice="disciple-tools-quick-action-custom-workflow">
                <p><?php echo esc_html( $message );?></p>
            </div>
            <script>
                jQuery(function($) {
                    $( document ).on( 'click', '.notice-disciple-tools-quick-action-custom-workflow .notice-dismiss', function () {
                        $.ajax( ajaxurl, {
                            type: 'POST',
                            data: {
                                action: 'dismissed_notice_handler',
                                type: 'disciple-tools-quick-action-custom-workflow',
 ...
 * @link https://github.com/DiscipleTools/disciple-tools-quick-action-custom-workflow/wiki/Configuring-Remote-Updating-System
 *
...
//                'https://raw.githubusercontent.com/DiscipleTools/disciple-tools-quick-action-custom-workflow/master/version-control.json',
//                __FILE__,
